...
function calculateHeuristic(schedule, startCoords) {
	...
	const currMaxSplit = _.max(schedule.metrics.split);
	const currMaxEarly = _.max(schedule.metrics.early);
	const currMaxLate = _.max(_.values(_.omit(schedule.metrics.late, schedule.TEAM_6_POS))); // OPTIMIZE max without team 6
