...
const DELAY_REMATCH = 4; // floor(weeks / 2); 3 is relaxed, 4 is normal, 5 is super strict
...
exports.Schedule = class Schedule {
	...
	init() {
		...
		this.remainingMatches = computeAllMatches(this).reverse();
		...
		for(let i=0; i<this.number_of_teams; i++) {
			this.metrics.early.push(0);
			this.metrics.late.push(0);
			this.metrics.split.push(0);
		}
	...
	clone() {
		...
		schedule.metrics = _.cloneDeep(this.metrics);
		return schedule;
	}
	...
	calcAllowableMatches({ week, time, arena }) {
		...
		Array.prototype.push.apply(pullMatches, _.chain(this.book)
			.slice(Math.max(0, week - DELAY_REMATCH), Math.min(week + DELAY_REMATCH + 1, this.number_of_weeks))
	...
	setMatch({ week, time, arena }, match) {
		...
		switch(time) {
			case 0: {
				const second = _.compact(_.flatten(this.book[week][1]));
				const third = _.compact(_.flatten(this.book[week][2]));
				if(_.includes(second, match[0])) this.metrics.early[match[0]-1]++;
				else if(_.includes(third, match[0])) this.metrics.split[match[0]-1]++;
				if(_.includes(second, match[1])) this.metrics.early[match[1]-1]++;
				else if(_.includes(third, match[1])) this.metrics.split[match[1]-1]++;
				break;
			}
			case 1: {
				const first = _.compact(_.flatten(this.book[week][0]));
				const third = _.compact(_.flatten(this.book[week][2]));
				if(_.includes(first, match[0])) this.metrics.early[match[0]-1]++;
				else if(_.includes(third, match[0])) this.metrics.late[match[0]-1]++;
				if(_.includes(first, match[1])) this.metrics.early[match[1]-1]++;
				else if(_.includes(third, match[1])) this.metrics.late[match[1]-1]++;
				break;
			}
			case 2: {
				const first = _.compact(_.flatten(this.book[week][0]));
				const second = _.compact(_.flatten(this.book[week][1]));
				if(_.includes(first, match[0])) this.metrics.split[match[0]-1]++;
				else if(_.includes(second, match[0])) this.metrics.late[match[0]-1]++;
				if(_.includes(first, match[1])) this.metrics.split[match[1]-1]++;
				else if(_.includes(second, match[1])) this.metrics.late[match[1]-1]++;
				break;
			}
		}
