...
exports.Schedule = class Schedule {
	...
	calcMetrics() {
		...
		return {
			...
			minEarly: _.min(_.values(_.omit(early, this.TEAM_6_POS))), // OPTIMIZE max without team 6
			maxLate: _.max(_.values(_.omit(late, this.TEAM_6_POS))), // OPTIMIZE max without team 6
			minLate: _.min(late),
			maxSplit: _.max(split),
			minSplit: _.min(_.values(_.omit(split, this.TEAM_6_POS))), // OPTIMIZE max without team 6
