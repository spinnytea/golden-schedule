...
/**
 * These are the hard and fast rules. What is a schedule and what is allowed.
 * No fancy heuristics or questions. Just facts and metrics.
 *
 * contains the matches that are already recorded and when
 * contains the matches that have yet to be filled in
 * computes which matches are allowed in a given slot, given the other matches already scheduled
...
exports.Schedule = class Schedule {
	...
	calcMetrics() {
		...
		// OPTIMIZE min/max without team 6
		// - make it the first or last team
		// - tighten the loop to ignore that position
		// - use swapTeams to swap it into place
		return {
			early,
			late,
			split,
			maxEarly: _.max(early),
			minEarly: _.min(_.values(_.omit(early, this.TEAM_6_POS))),
			maxLate: _.max(_.values(_.omit(late, this.TEAM_6_POS))),
			minLate: _.min(late),
			maxSplit: _.max(split),
			minSplit: _.min(_.values(_.omit(split, this.TEAM_6_POS))),
	...
	// XXX this method is implemented exactly for our use case
	prettyBookCsv(headers) {
		let str = '';

		if(!headers || !headers.time || !headers.arena)
			throw new Error('must provide headers for week, time, arena');
		if(headers.time.length !== this.number_time_slots_per_week)
			throw new Error('incorrect number of times: expected ' + this.number_time_slots_per_week + ', actual: ' + headers.time.length);
		if(headers.arena.length !== this.number_concurrent_games)
...
function computeAllMatches({ number_of_teams, number_of_pairing_matches }) {
	...
	void(number_of_pairing_matches);
	// for(let a = 1; a <= number_of_teams; a++)
	// 	for(let b = a+1; b <= number_of_teams; b++)
	// 		for(let p=0; p < number_of_pairing_matches; p++)
	...
	// FIXME flag to shuffle computed games
	// - can we just do that on the schedule once we have it constructed?
	// return _.shuffle(matches);
	return matches;
}
