...
exports.BookIterator = class BookIterator {
	...
	// IDEA iterate over time as 0, 2, 1
	// - not sure if it will actually help, all time slots matter, not just split
	// - we are optimizing split, but won't we optimize the others, too?
	// - if we go out of order, we need to double check literally all other code, much of it assumes 'in-order' iteration
	// - maybe all the 'partial iterations' are with bookIterator, and all the non-bookIterator are 'complete iterations'
	next() {
		if(!this.done) {
			this.arena++;
